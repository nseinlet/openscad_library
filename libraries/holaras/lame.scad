...
module _half_middle_blade(angle){
  difference(){
    hull(){
    ...
    for (z=[6, blade_height-3]) for (x=hinge_holes) translate([(blade_width/2)-x, blade_thickness-0.1, z]) rotate([90, 0, 0]) cylinder(h=blade_thickness*2, r=1.01, center=true);
    // 3rd point hole
    for (posx=[20, 45]) translate([posx,0,17]) rotate([90,0,0]) cylinder(r=1, h=blade_thickness*2, center=true);
  }
}

module middle_blade(angle=0){
  union(){
    _half_middle_blade(angle);
    mirror([1,0,0]) _half_middle_blade(angle);
  }
}
...
module side_blade(){
  difference(){
    ...
    for (x=[14:5:40]) for (pos_z=[blade_height+upper_blade_height-3, blade_height-11]){
        hull(){
            translate([(upper_side_blade_width/2)-x, blade_thickness-0.1, pos_z-upper_blade_height+6]) rotate([90, 0, 0]) cylinder(h=blade_thickness*2, r=1.25, center=true);
            translate([(upper_side_blade_width/2)-x, blade_thickness-0.1, pos_z]) rotate([90, 0, 0]) cylinder(h=blade_thickness*2, r=1.25, center=true);
        }
    }
    for (z=[10, blade_height-7]) for (x=hinge_holes) translate([side_blade_width-x, blade_thickness-0.1, z]) rotate([90, 0, 0]) cylinder(h=blade_thickness*2, r=1.01, center=true);
    // Hinge fix
...
module hinge(){
    difference(){
        ...
        for (x=hinge_holes) translate([x, 0, 2]) rotate([90, 0, 0]) cylinder(h=blade_thickness+12, r=1.01, center=true);
...
module three_points_linkage_blade_fix(){
  difference(){
    union(){
      translate([0,-72.5,0]) cube([110, 2, 10], center=true);
      for (pos_x=[-45, -20, 20, 45]) translate([pos_x, -72.5, 0]) cube([6, 2, 14], center=true);
    }
    for (pos_x=[-45, -20, 20, 45]) translate([pos_x, -72.5, 0]) rotate([90,0,0]) cylinder(r=1.1, h=5, center=true);
  }
}
...
module three_points_linkage_third_point(){
  difference(){
    union(){
      translate([6, -10, 0]) cube([2, 4, 15], center=true);
...
module rotation_arm1(){
  ...
  difference(){
    union(){
      hull(){
        translate(begin) cylinder(r=2, h=2, center=true);
    //    translate(middle) cylinder(r=2, h=2, center=true);
    //  }
    //  hull(){
    //    translate(middle) cylinder(r=2, h=2, center=true);
        translate(end) cylinder(r=2, h=2, center=true);
      }
      translate(begin) cylinder(r=3, h=2, center=true);
      translate(end) cylinder(r=3, h=2, center=true);
    }
    //holes
    translate(begin) cylinder(r=1.5, h=2.2, center=true);
    translate(end) cylinder(r=1.5, h=2.2, center=true);
  }
}
